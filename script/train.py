...
def load_word_index(path):
    """
    Wordnet for the corpus; has the mapping of each owrd to unique token
    :return: word_idex
    """
...
def load_emb_matrix(path):
    ...
    print('Unnormalised Shape: ({},{})'.format(np.shape(embedding_matrix)[0], np.shape(embedding_matrix)[1]))
    return embedding_matrix
...
def prec_rec_f1_score(model, x, y_true):
    ...
    print('loss', loss)
    return accuracy, precision_true, precision_fake, recall_true, recall_fake, f1score_true, f1score_fake, round(loss,3)

def reports(path, model, ratio, lr, batch_size, activation, n_epoch, epsilon, doc_length, embedding_dim, train_accuracy, test_accuracy,
                    precision_true, precision_fake, recall_true, recall_fake, f1_score_true, f1_score_fake, loss, comment):

    time_now = datetime.datetime.now()
    report = pd.DataFrame(columns=['Date', 'Model', 'Ratio (test/train)' 'Learning Rate', 'Batch Size', 'Activation', 'Epochs', 'Epsilon', 'Document Length',
                                   'Embedding dim', 'Training Accuracy', 'Testing Accuracy', 'Precision True', 'Precision False',
...
def supervised(lr = 0.002, n_epochs = 4, act_fn = 'tanh', batch_size=32, train_dataset=None, test_dataset=None):
    optimizer = tf.keras.optimizers.Adam(learning_rate=lr, beta_1=0.9, beta_2=0.999, epsilon=1e-07,
                                         amsgrad=False, name='Adam')
    loss_object = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=False)

    model_supervised = reset_model(act_fn)
    dataset_batch = 1
    last_accuracy_list = []
    ...
    train_accuracy = round(np.mean(last_accuracy_list),3)
...
def vat_carrier(p_logit, clean_emb):
    ...
    with tf.GradientTape(watch_accessed_variables=False) as tape:
        ...
        #print('Returned Kl Score:', kl_score)
    grads = tape.gradient(kl_score, noise_emb)
    #print('Grads: First',tf.reduce_sum(grads), grads.shape)
    p_logit = tf.stop_gradient(p_logit)
    grads = tf.stop_gradient(grads)

    #print('Grads: Before',tf.reduce_sum(grads), grads.shape)
    norm_ball_2 = tf.math.l2_normalize(grads, axis=0)
    #print('Grads: After',tf.reduce_sum(norm_ball_2), norm_ball_2.shape)
    r_vadv = (grads/norm_ball_2) * -1
    #print('r_vadv',tf.reduce_sum(r_vadv), r_vadv.shape)

    r_vadv_emb = tf.add(clean_emb, r_vadv)
    q_logit = network(r_vadv_emb)
    vat_loss = compute_kld(p_logit, q_logit)
    #print('Final Vat loss: ', vat_loss)
    return vat_loss

def vat(lr = 0.002, n_epochs = 4, act_fn = 'tanh', batch_size=32, train_dataset=None, test_dataset=None):
    optimizer = tf.keras.optimizers.Adam(learning_rate=lr, beta_1=0.9, beta_2=0.999, epsilon=1e-07,
                                         amsgrad=False, name='Adam')
    loss_object = tf.keras.losses.SparseCategoricalCrossentropy(from_logits=False)

    model_vat = reset_model(act_fn)
    dataset_batch = 1
    last_accuracy_list = []
    ...
    for features, labels in train_dataset:
        ...
        vat_loss = vat_carrier(p_logit, clean_emb)
...
if __name__ == '__main__':
    ...
    x_train, y_train, x_test, y_test = load_data(path + '/temp/10k')
    ...
    comment = 'Initial Testing 10 Fold Results'
    ...
    for train_ind, test_ind in kf.split(X):
        ...
        if model_type == 'Supervised':
            ...
            f1_score_true, f1_score_fake, classification_loss, model = supervised(lr, n_epochs, act_fn, batch_size,
